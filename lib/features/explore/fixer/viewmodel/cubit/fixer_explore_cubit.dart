...
void _applyFiltersToState() {
  ...
  if (state.selectedSkills.isNotEmpty) {
    final matchingSkillTasks = tasks.where((task) =>
      task.skills.any((skill) => state.selectedSkills.contains(skill))
    );

    if (matchingSkillTasks.isNotEmpty) {
      minBudget = matchingSkillTasks.map((t) => t.budget).reduce((a, b) => a < b ? a : b);
      maxBudget = matchingSkillTasks.map((t) => t.budget).reduce((a, b) => a > b ? a : b);
    }
  }
  ...
  emit(state.copyWith(
    ...
    priceRangeStart: state.priceRangeStart.clamp(minBudget, maxBudget),
    priceRangeEnd: state.priceRangeEnd.clamp(minBudget, maxBudget),
  ));
}


List<TaskPostModel> applyFilters(List<TaskPostModel> tasks, FixerExploreState state) {
  ...
  if (state.searchQuery.isNotEmpty) {
    filtered = filtered.where((task) =>
      ...
      (task.description?.toLowerCase().contains(state.searchQuery.toLowerCase()) ?? false)
    ).toList();
  ...
  if (state.selectedSkills.isNotEmpty) {
    ...
    if (matchingSkillTasks.isNotEmpty) {
      final minBudget = matchingSkillTasks.map((t) => t.budget).reduce((a, b) => a < b ? a : b);
      final maxBudget = matchingSkillTasks.map((t) => t.budget).reduce((a, b) => a > b ? a : b);

      debugPrint('Dynamic price range for ${state.selectedSkills.join(", ")}: $minBudget - $maxBudget');

      // Clamp the user-selected range to this dynamic range
      filtered = filtered.where((task) {
        final passes = task.budget >= state.priceRangeStart &&
                       task.budget <= state.priceRangeEnd;
        if (!passes) {
          debugPrint('Filtered out ${task.title} due to price (${task.budget})');
        }
        return passes;
      }).toList();
    }
  ...
  if (state.selectedSkills.isNotEmpty) {
    filtered = filtered.where((task) {
      final hasMatchingSkill = task.skills.any((skill) =>
        state.selectedSkills.contains(skill));
      if (!hasMatchingSkill) {
